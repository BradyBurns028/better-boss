...
export default defineNuxtComponent({
    ...
    methods: {
        async getDegreePrograms(this: any) {
            ...
            this.degreePrograms = await degreeProgramApi.list(1, 1000)
            if(needle.length > 0){
                this.degreePrograms = this.degreePrograms.filter((program: any) =>
                    program.name.toLowerCase().includes(needle) ||
                    program.department.name.toLowerCase().includes(needle)
                )
            }
            this.loading = false;
        },
